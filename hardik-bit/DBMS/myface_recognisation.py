...
class DetectFace:
    def __init__(self, win_tk_root):  # constructor
        ...
        left_image = Image.open(r"img\girl.JPEG")
        ...
        top_right_image = Image.open(r"img\girl.JPEG")
        ...
        bottom_right_image = Image.open(r"img\clg.jpg")
    ...
    def draw_boundary(self, img, classifier, scaleFactor, minNeighbors, color, text, clf):
        ...
        for (x, y, w, h) in features:
            ...
            conn = mysql.connector.connect(host = "localhost", username = "root", password = "7404983564@Ge",
                                           database = "attendance_database")
            my_cursor = conn.cursor()

            my_cursor.execute("select Name from students where USN=" + str(r_id))
            n = my_cursor.fetchone()
            n = "+".join(n)

            my_cursor.execute("select Department from student where USN=" + str(r_id))
            d = my_cursor.fetchone()
            d = "+".join(d)

            my_cursor.execute("select Course from student where USN=" + str(r_id))
